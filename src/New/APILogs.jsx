...
// Enhanced Sidebar Component (from APIEventDashboard.jsx)
const Sidebar = ({ isOpen, setIsOpen }) => {
  const { colors } = useTheme();
  
  const menuItems = [
    { icon: Home, label: 'Dashboard', active: false },
    { icon: Activity, label: 'Event Monitor', active: true },
  ...
  return (
    <aside 
      ...
      className={`${isOpen ? 'w-64' : 'w-20'} transition-all duration-300 h-screen border-r fixed left-0 top-0 z-10`}
    >
      <div className="flex flex-col h-full">
        <div className="h-16 flex items-center justify-between px-4">
          ...
          <button 
            ...
            onMouseEnter={(e) => e.target.style.backgroundColor = colors.sidebarHover}
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <button 
                ...
                className="w-full flex items-center gap-4 px-4 py-3 rounded-lg mb-1 transition-all"
                onMouseEnter={(e) => {
                  if (!item.active) {
                    e.target.style.backgroundColor = colors.sidebarHover;
